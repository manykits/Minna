...
class AsrWorker:
    ...
    def reset(self):
        ...
        self.listening = True
        self.content = ''
        print("resettttttttttttttttttttttttttttttt")
    ...
    def is_exit(self):
        ...
        match_tokens  = ['退下', '退一下','退出', '拜拜', '再见', '晚安', '退下。', '退一下。', '退出。', '拜拜。', '再见。', '晚安。']
        words = jieba.lcut(self.content)  # 对文本进行分词
        words = [word.lower() for word in words]  # 转小写
        for token in match_tokens:
            if token in words:
                return True
        return False

        # match_tokens = ['退下', '退出', '拜拜', '再见', '晚安']
        # last_part = self.content.lower()
        # last_part = last_part.strip()  # 去除首尾空白字符
        # for token in match_tokens:
        #     dist = distance(last_part, token)
        #     print("dist:")
        #     print(dist)
        #     if dist <= 1:  # 这里设定编辑距离阈值为2，可根据实际情况调整
        #        return True
    ...
    def reply(self):
        ...
        if self.is_exit():
            print("is_exitttttttttttttttttttttttttttttttttttttttttttttttttt")
        ...
        allurl = os.path.join(oss_prefix, oss_key)
    ...
    def stop(self):
        ...
        if self.audio_buffer.shape[0] > 0:
            self.content = self.generate_text()
            self.reply()
        print("stoppppppppppppppppppppppppppppppppp")
